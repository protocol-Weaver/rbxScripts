...
local abilities = {
    ...
    Flight = { cooldown = 3, staminaCost = 10 }
}
local lastAbilityTime = { Dash = 0, Slash = 0, Flight = 0 }

-- Stamina Management
local maxStamina = 100
local stamina = maxStamina
local staminaRegenRate = 5 -- Per second
...
local function onStateChanged(oldState, newState)
    -- Update the isJumping flag based on the new state
    isJumping = (newState == Enum.HumanoidStateType.Jumping or 
...
local function toggleFlight()
    -- Check if the ability can be used
    if not canUseAbility("Flight") then return end

    -- Ensure the player is in a freefall state before enabling flight
    if not isJumping or humanoid:GetState() ~= Enum.HumanoidStateType.Freefall then return end
    
    -- Create body velocity and align orientation for flight control
    local bodyVelocity = Instance.new("BodyVelocity")
    local alignOrientation = Instance.new("AlignOrientation")

    -- Set the max force for body velocity to control movement
    ...
    -- Enable or disable flight components based on the flying state
    ...
    -- Disable character animation while flying
    character.Animate.Disabled = flying

    -- If flying is enabled, control flight based on user input
    if flying then
        while flying do
            -- Get the movement vector from the control module
            local moveVector = controlModule:GetMoveVector()
            
            -- Calculate the direction based on camera orientation and movement input
            ...
            -- Align the character's orientation with the camera and set flight velocity
